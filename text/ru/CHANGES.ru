...
Изменения в freenginx 1.31.2                                      26.05.2026
    ...
    *) Исправление: директива if при проверке файлов некорректно
       обрабатывала относительные пути.


Изменения в freenginx 1.31.1                                      19.05.2026

    *) Добавление: параметр off в директиве index.
       Спасибо Fabiano Furtado.

    *) Исправление: в рабочем процессе мог произойти segmentation fault,
       если директива rewrite использовалась для изменения аргументов
       запроса и после неё выполнялись другие директивы модуля
    ...
    *) Исправление: в модуле ngx_http_charset_module.
    ...
    *) Исправление: в директиве scgi_request_buffering.

    *) Исправление: в HTTP/3.


Изменения в freenginx 1.31.0                                      05.05.2026

    *) Изменение: теперь директиву try_files можно использовать с
       аргументами, начинающимися на URI запроса, внутри location, заданного
       регулярным выражением, вместе с директивой alias.

    *) Исправление: директива try_files могла работать некорректно при
       совместном использовании с директивой alias.
    ...
    *) Исправление: после изменения URI запроса с помощью директивы
       try_files директива proxy_pass без URI могла использовать URI
       исходного запроса.

    *) Изменение: уровень логгирования ошибок SSL "invalid ccs message",
       "not on record boundary", "required compression algorithm missing" и
       некоторых ошибок "record layer failure" понижен с уровня crit до
       info.


Изменения в freenginx 1.29.7                                      31.03.2026

    *) Добавление: совместимость с OpenSSL 4.0.

    *) Исправление: в рабочем процессе мог произойти segmentation fault,
       если модуль ngx_http_dav_module использовался для обработки методов
       COPY и MOVE в location с директивой alias.

    *) Исправление: при использовании директивы resolver в SMTP
       прокси-сервере имя хоста клиента не экранировалось при передаче на
       сервер аутентификации и в команде XCLIENT.

    *) Исправление: в почтовом прокси-сервере.


Изменения в freenginx 1.29.6                                      10.03.2026

    *) Исправление: при использовании переменных в proxy_pass в лог могли
       попадать некорректные сообщения "upstream server temporarily
       disabled".

    *) Исправление: повторная отправка запроса на другой gRPC-бэкенд могла
       работать некорректно.
       Спасибо David Carlier.

    *) Исправление: в рабочем процессе мог произойти segmentation fault,
       если использовался модуль ngx_http_xslt_filter_module.

    *) Исправление: в рабочем процессе мог произойти segmentation fault,
       если использовался модуль ngx_http_mp4_module.

    *) Исправление: в модулях ngx_http_uwsgi_module и ngx_http_scgi_module.
...
Изменения в freenginx 1.29.4                                      16.12.2025
    ...
    *) Добавление: поддержка баз данных GeoIP2 в формате MaxMind DB (MMDB) в
       модулях ngx_http_geoip_module и ngx_stream_geoip_module; директива
       geoip_set.

    *) Исправление: размер кэша на диске мог считаться некорректно при
       использовании файловой системы XFS с опцией монтирования "largeio".
...
Изменения в freenginx 1.27.4                                      03.09.2024

    *) Добавление: переменная $ssl_client_fingerprint_sha256.

    *) Добавление: теперь серверу аутентификации почтового прокси-сервера
       передаётся строка заголовка "Auth-SSL-Fingerprint-SHA256".

    *) Изменение: MIME-тип для расширения js изменён на "text/javascript",
       для расширения mjs теперь используется тип "text/javascript", для
       расширений md и markdown теперь используется тип "text/markdown";
       значение по умолчанию директивы charset_types теперь включает
       "text/javascript" и "text/markdown".

    *) Исправление: в рабочем процессе мог произойти segmentation fault,
       если использовался модуль ngx_http_mp4_module; ошибка появилась в
       1.5.13.
...
Изменения в freenginx 1.27.3                                      13.08.2024

    *) Изменение: теперь при кэшировании учитывается строка "Age" заголовка
       ответа бэкенда.
       Спасибо Hiroaki Nakamura.

    *) Добавление: переменная $upstream_cache_age.
...
Изменения в freenginx 1.27.2                                      09.07.2024

    *) Добавление: теперь максимальная частота логгированиия ошибок может
       быть ограничена с помощью параметра rate директивы error_log; по
       умолчанию используется ограничение в 1000 сообщений в секунду.
...
Изменения в freenginx 1.27.1                                      04.06.2024
    ...
    *) Добавление: переменная $upstream_cache_key.
       Спасибо Кириллу Коринскому.

    *) Добавление: поддержка методов аутентификации XOAUTH2 и OAUTHBEARER в
       почтовом прокси-сервере.
       Спасибо Rob Mueller.

    *) Исправление: плавное завершение старых рабочих процессов могло
       задерживаться при использовании HTTP/2.
       Спасибо Kasei Wang.

    *) Исправление: при использовании HTTP/3 в рабочем процессе мог
...
Изменения в freenginx 1.27.0                                      14.05.2024
    ...
    *) Исправление: перенаправление ошибок 413 с помощью директивы
...
Изменения в freenginx 1.25.4                                      20.02.2024

    *) Изменение: теперь в ответах используется имя "freenginx".

    *) Исправление: при использовании AIO во время завершения рабочих
       процессов в логах могли появляться сообщения "open socket left".

    *) Исправление: при использовании AIO в подзапросах в рабочем процессе
       мог произойти segmentation fault.

    *) Исправление: в рабочем процессе мог произойти segmentation fault,
       если использовалось директива image_filter, а ошибки с кодом 415
       перенаправлялись с помощью директивы error_page.

    *) Исправление: при обработке закэшированных ответов с заголовком
       X-Accel-Redirect в рабочем процессе мог произойти segmentation fault.
       Спасибо Jiří Setnička.
...
Изменения в nginx 0.3.53                                          07.07.2006
    ...
    *) Исправление: если при использовании "satisfy_any on" клиент успешно
       проходил аутентификацию, в лог всё равно записалось сообщение "access
       forbidden by rule".
...
Изменения в nginx 0.3.17                                          18.12.2005
    ...
    *) Исправление: во время SSL handshake не использовались таймауты;
